...
const HotelCard = ({ hotel }) => {
  return (
    <div className="bg-white h-[14rem] w-[107%] shadow font-poppins rounded-lg overflow-hidden flex flex-col md:flex-row">
      ...
      <div className="p-4 w-full md:w-1/3">
        <h2 className="text-xl font-semibold mb-2 text-black">{hotel.Hotel_Name}</h2>
  ...
  {hotel.Amenities.map((amenity, index) => (
    <li key={index}>{amenity}</li>
        ...
        <button className="bg-[#90CCBA] hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
          Book Now
        </button>
      </div>
    </div>
